...
for (b in seq(B)) {
  ...
  Q_aug_fit <- cv_fit_hal_augment_safe(X = data.frame(W, A = A),
                                       Y = Y,
                                       basis_list = Q_basis_list,
                                       col_idx = c(1, 2),
  ...
  # make unpenalized design matrix
  X_unpenalized_A1 <- as.matrix(make_design_matrix(X = as.matrix(data.frame(W, A = 1)),
                                                   blist = Q_aug_fit$basis_list_non_zero))
  X_unpenalized_A0 <- as.matrix(make_design_matrix(X = as.matrix(data.frame(W, A = 0)),
                                                   blist = Q_aug_fit$basis_list_non_zero))

  Q1_aug_est <- predict(Q_aug_fit$fit, new_data = data_A1, new_X_unpenalized = X_unpenalized_A1)
  Q0_aug_est <- predict(Q_aug_fit$fit, new_data = data_A0, new_X_unpenalized = X_unpenalized_A0)
